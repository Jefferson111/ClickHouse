...
inline size_t countCodePoints(const UInt8 * data, size_t size)
{
    ...
    constexpr auto bytes_sse = sizeof(simde__m128i);
    const auto src_end_sse = data + size / bytes_sse * bytes_sse;

    const auto threshold = simde_mm_set1_epi8(0xBF);

    for (; data < src_end_sse; data += bytes_sse)
        res += __builtin_popcount(simde_mm_movemask_epi8(
            simde_mm_cmpgt_epi8(simde_mm_loadu_si128(reinterpret_cast<const simde__m128i *>(data)), threshold)));
